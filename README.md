...
- [ ] Код полностью решает поставленную задачу
- [ ] Обработаны граничные случаи (null, пустые коллекции, экстремальные значения)
...
- [ ] Имена переменных, методов и классов понятные и отражают назначение
...
**Проблема:** Краткое описание проблемы с указанием файла и строки

**Почему это важно:**
Объяснение последствий (ошибка, плохая читаемость, потенциальный баг и т.д.)

**Предложение:**
Конкретное решение или рекомендация
...
Проблема: В методе calculate() не обрабатывается null (Calculator.java:25)

Почему это важно:
Это может привести к NullPointerException при передаче некорректных данных

Предложение:
Добавить проверку на null и выбрасывать IllegalArgumentException
